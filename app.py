...
def show_unified_business_search():
    ...
    if "biz_step1_df" in st.session_state:
        ...
        if st.button("🚀 STEP 4. 자동 추출 시작 (행정망 중심 데이터 수집)", type="primary", use_container_width=True):
            from api.nps_api import search_nps_by_name, search_nps_by_region
            ...
            try:
                search_inds = selected_industries if selected_industries else [
                    # 1. 경영·기획·사무·인사·회계 (30개)
                    "기획", "전략", "경영지원", "총무", "인사", "채용", "노무", "급여", "회계", "세무", "경리", "재무", "자금", "IR", "감사", "비서", "사무", "행정", "홍보", "브랜딩", "광고기획", "시장조사", "컨설팅", "특허", "법률", "법무", "관세", "특허사무소", "법률사무소", "세무회계",
                    
                    # 2. IT·개발·인프라·보안 (30개)
                    "테크", "코리아", "시스템", "정보통신", "소프트웨어", "디지털", "연구소", "과학", "솔루션", "플랫폼", "SI", "ERP", "MES", "CRM", "서버", "네트워크", "클라우드", "데이터", "인프라", "보안", "백엔드", "프론트엔드", "앱개발", "웹개발", "게임개발", "인공지능", "빅데이터", "호스팅", "정보보호", "통신장비",
                    
                    # 3. 마케팅·광고·디자인·미디어 (30개)
                    "마케팅", "광고", "디자인", "기획", "인쇄", "출판", "미디어", "콘텐츠", "엔터테인먼트", "영상", "편집", "웹디자인", "그래픽", "일러스트", "캐릭터", "카피라이팅", "프로모션", "이벤트", "전시", "행사기획", "스튜디오", "사진", "인테리어디자인", "패션디자인", "VMD", "제품디자인", "애니메이션", "속기", "기자", "신문",
                    
                    # 4. 영업·CS·도소매·무역·유통 (35개)
                    "영업", "기술영업", "해외영업", "무역", "도매", "소매", "도소매", "상사", "유통", "납품", "공급", "대리점", "총판", "특판", "수출입", "글로벌", "종합", "마트", "슈퍼", "백화점", "아울렛", "쇼핑몰", "이커머스", "MD", "상품기획", "소싱", "물류대행", "3PL", "포워딩", "고객센터", "CS", "콜센터", "인바운드", "아웃바운드", "리셉션",
                    
                    # 5. 구매·자재·물류·운송 (25개)
                    "구매", "자재", "물류", "창고", "운송", "화물", "택배", "배송", "퀵서비스", "지입", "포장", "패키징", "박스", "종이", "펄프", "가죽", "원단", "섬유", "원사", "의류", "패션", "완구", "문구", "잡화", "보세",
                    
                    # 6. 생산·제조·화학·바이오·연구 (35개)
                    "제조", "공장", "산업", "종합", "생산", "품질", "설비", "기계", "금형", "사출", "프레스", "도금", "용접", "정밀가공", "금속가공", "와이어", "케이블", "파이프", "밸브", "모터", "베어링", "기어", "센서", "패널", "제어기", "컴프레셔", "자동화", "반도체", "디스플레이", "PCB", "SMT", "화학", "플라스틱", "고무", "세라믹",
                    
                    # 7. 건설·토목·조경·환경·에너지 (20개)
                    "건설", "건축", "토목", "조경", "시공", "시행", "엔지니어링", "플랜트", "소방", "안전", "전기공사", "설비공사", "환경", "폐기물", "에너지", "발전", "태양광", "철강", "금속", "알루미늄",
                    
                    # 8. 의료·보건·제약·복지·요양 (25개)
                    "병원", "의원", "치과", "한의원", "약국", "임상", "제약", "바이오", "메디칼", "의료기기", "요양", "요양원", "요양병원", "간병", "보호센터", "사회복지", "복지관", "복지재단", "의학", "안과", "내과", "피부과", "소아과", "이비인후과", "정형외과",
                    
                    # 9. 교육·학원·스포츠·여가·관광 (25개)
                    "학원", "교육", "학교", "어린이집", "유치원", "보육", "스터디", "독서실", "헬스", "피트니스", "필라테스", "요가", "골프", "스포츠", "레저", "체육관", "수영장", "테니스", "여행", "관광", "숙박", "호텔", "펜션", "캠핑", "레포츠",
                    
                    # 10. 외식·음료·전문서비스 (20개)
                    "식품", "푸드", "식당", "맛집", "카페", "커피", "디저트", "베이커리", "제과", "반찬", "식자재", "고기", "육류", "수산물", "급식", "프랜차이즈", "도시락", "요리", "조리", "바리스타"
                ]
            ...
            for idx, c_data in enumerate(scraped_list):
                ...
                existing_idx = None
                if k1 and k1 in final_results_dict: existing_idx = final_results_dict[k1]
            ...
            if final_results:
                ...
                st.success(f"✅ 총 {len(final_results)}개 업체 발굴 완료! 아래 결과를 확인하세요.")
            else:
                st.warning(f"근로자 수({min_jnngp}명 이상) 조건을 만족하는 업체가 없습니다.")
